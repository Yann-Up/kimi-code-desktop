...
export function QuestionCard({ question }: { question: PendingQuestion }) {
  ...
  const canSubmit = question.questions.every(
    (q) => (selected[q.id]?.length ?? 0) > 0 || (other[q.id]?.trim().length ?? 0) > 0
  )
  ...
  return (
    <div className="rounded-xl border border-primary-border bg-primary-soft/60 p-4">
      ...
      <button
        className="mt-3 rounded-lg bg-primary px-4 py-1.5 text-[13px] font-medium text-white hover:bg-primary-hover disabled:opacity-40"
        disabled={!canSubmit}
        onClick={submit}
      >
        提交回答
