...
- **端口稳定(源即身份)**:web UI 的"新浏览器"验证状态按 iframe 源(`http://127.0.0.1:<port>`)存 localStorage,端口漂移就会重弹验证;故固定从 58666 起,且启动前先回收首选端口上的残留实例(应用崩溃/强杀留下的孤儿,POST shutdown + 注册表 pid 强杀兜底)保证该端口可用;其他端口上用户另开的 kimi web 实例不动
